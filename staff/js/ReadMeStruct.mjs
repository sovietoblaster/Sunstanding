...
export class ReadMeStruct {
    ...
    #createLine(chapter, index) {
        return this.#lineTemplateMatch[0]
            ...
            .replace(new RegExp(T_4_NAME, 'ud'), `${this.#filePath}/${chapter.code} ${chapter.title}.pdf`)
            .replace(new RegExp(T_4_PATH, 'ud'), `${this.#filePath}/${chapter.code} ${chapter.title}.pdf`.replace(new RegExp(` `, 'udg'), `%20`))
            ;
    }
